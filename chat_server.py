...
from flask_socketio import disconnect as socketio_disconnect_client  # Dodaj ten import
from flask import session  # Dodaj 'session'
...
UPLOAD_FOLDER = os.path.join("data", "chat", "uploads")
LEGACY_UPLOAD_FOLDERS = ["chat_uploads", "uploads"]
ALLOWED_EXTENSIONS = {
    "txt",
    "pdf",
    "png",
...
)  # Ustawienie limitu w bajtach


def _migrate_legacy_upload_folders():
    target_upload_dir = app.config["UPLOAD_FOLDER"]
    os.makedirs(target_upload_dir, exist_ok=True)

    for legacy_folder in LEGACY_UPLOAD_FOLDERS:
        legacy_upload_dir = os.path.join(os.getcwd(), legacy_folder)
        if not os.path.isdir(legacy_upload_dir):
            continue

        for file_name in os.listdir(legacy_upload_dir):
            source_path = os.path.join(legacy_upload_dir, file_name)
            if not os.path.isfile(source_path):
                continue

            target_path = os.path.join(target_upload_dir, file_name)
            if os.path.exists(target_path):
                base_name, ext = os.path.splitext(file_name)
                target_path = os.path.join(
                    target_upload_dir,
                    f"{base_name}_legacy_{uuid.uuid4().hex[:8]}{ext}",
                )

            try:
                os.replace(source_path, target_path)
                logger.info(f"Zmigrowano plik uploadu: {source_path} -> {target_path}")
            except OSError as error:
                logger.warning(
                    f"Nie udało się przenieść uploadu '{source_path}' do '{target_path}': {error}"
                )

        try:
            if not os.listdir(legacy_upload_dir):
                os.rmdir(legacy_upload_dir)
        except OSError:
            pass


_migrate_legacy_upload_folders()

# Konfiguracja bazy danych SQLite
CHAT_DATA_DIR = os.path.join(os.getcwd(), "data", "chat")
CHAT_DB_PATH = os.path.join(CHAT_DATA_DIR, "chat.db")
LEGACY_CHAT_DB_PATH = os.path.join(os.getcwd(), "chat.db")

if not os.path.exists(CHAT_DB_PATH) and os.path.exists(LEGACY_CHAT_DB_PATH):
    os.makedirs(CHAT_DATA_DIR, exist_ok=True)
    try:
        os.replace(LEGACY_CHAT_DB_PATH, CHAT_DB_PATH)
        logger.info(f"Zmigrowano bazę czatu do: {CHAT_DB_PATH}")
    except OSError as error:
        logger.warning(
            f"Nie udało się zmigrować bazy czatu do '{CHAT_DB_PATH}': {error}"
        )

os.makedirs(CHAT_DATA_DIR, exist_ok=True)
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + CHAT_DB_PATH
app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = (
    False  # Wyłączamy, by zużywać mniej pamięci
)
app.config["SECRET_KEY"] = (
    "twoj_sekretny_klucz_ktory_musisz_zmienic!"  # WAŻNE: ZMIEŃ NA UNIKATOWY KLUCZ
)
...
ADMIN_PASSWORD_PLAIN = os.environ.get(
    "FLASK_ADMIN_PASS", "supersecretpassword"
)  # Hasło w postaci jawnej
...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not session.get("is_admin_logged_in"):
            ...
            return (
                jsonify({"error": "Administrator access required. Please log in."}),
                401,
            )  # Unauthorized
...
def admin_remove_from_email_blacklist(email_to_unblacklist):
    ...
    if not entry_to_remove:
        return (
            jsonify(
                {"error": f"Email '{validated_email}' not found on the blacklist."}
            ),
            404,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Email '{validated_email}' has been removed from the blacklist."
                }
            ),
            200,
        )
        ...
        return (
            jsonify({"error": "Server error during email unblacklisting operation"}),
            500,
        )
...
def admin_add_to_email_blacklist():
    ...
    if existing_entry:
        return (
            jsonify(
                {"error": f"Email '{validated_email}' is already on the blacklist."}
            ),
            409,
        )  # Conflict
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"Email '{validated_email}' has been added to the blacklist."
                }
            ),
            201,
        )  # Created
        ...
        return (
            jsonify({"error": "Server error during email blacklisting operation"}),
            500,
        )
...
def admin_index():
    if session.get("is_admin_logged_in"):
        ...
        return render_template_string(
            ADMIN_PANEL_HTML
        )  # Zawsze serwuj główny HTML panelu
...
def admin_get_all_users():
    try:
        ...
        users_data = []
        online_user_ids = set(
            connected_sids_to_user_id.values()
        )  # Pobierz zbiór ID online
        for user in users:
...
def admin_ban_user(user_id):
    try:
        ...
        if user_to_ban.is_admin:
            return (
                jsonify({"error": "Cannot ban an admin account through this panel."}),
                400,
            )
        ...
        sids_for_banned_user = [
            sid for sid, uid in connected_sids_to_user_id.items() if uid == user_id
        ]
        for sid_to_disconnect in sids_for_banned_user:  # Zmieniono nazwę zmiennej
            ...
            try:
                ...
                socketio_disconnect_client(
                    sid_to_disconnect, namespace="/", silent=False
                )  # Użyj namespace, jeśli masz
                logger.info(
        ...
        return (
            jsonify(
                {
                    "message": f"User '{user_to_ban.username}' has been banned and notified.",
...
def admin_unban_user(user_id):
    try:
        ...
        for sid in sids_for_unbanned_user:
            socketio.emit(
                "account_status_changed",
                {"banned": False, "reason": "Twoje konto zostało odblokowane."},
                room=sid,
            )
...
def admin_delete_user(user_id):
    try:
        ...
        username_deleted_for_event = (
            user_to_delete.username
        )  # Zapamiętaj nazwę dla eventu

        # Usuwanie relacji i wiadomości (bez zmian, ale upewnij się, że działa)
        BlockedRelationship.query.filter(
            (BlockedRelationship.blocker_id == user_id)
            | (BlockedRelationship.blocked_id == user_id)
        ...
        for sid_to_disconnect_del in sids_for_deleted_user:
            ...
            socketio.emit(
                "account_deleted",
                {"user_id": user_id, "username": username_deleted_for_event},
                room=sid_to_disconnect_del,
            )
            # --- POPRAWKA: Użyj zaimportowanej funkcji disconnect ---
            try:
                socketio_disconnect_client(
                    sid_to_disconnect_del, namespace="/", silent=False
                )
                logger.info(
        ...
        # --- POPRAWKA: Użyj poprawnej nazwy zmiennej dla username ---
        socketio.emit(
            "user_offline", {"user_id": user_id, "username": username_deleted_for_event}
        )
        logger.debug(
        ...
        return (
            jsonify(
                {
                    "message": f"User '{username_deleted_for_event}' and associated data has been deleted."
                }
            ),
            200,
        )
...
class EmailBlacklist(db.Model):
    ...
    )  # Unikalny zablokowany email
    reason = db.Column(
        db.String(255), nullable=True
    )  # Opcjonalny powód dodania do blacklisty
...
class BlockedRelationship(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("blocker_id", "blocked_id", name="_blocker_blocked_uc"),
    )

    blocker = db.relationship(
        "User", foreign_keys=[blocker_id], backref="blocking_relationships"
    )
    blocked_user = db.relationship(
        "User", foreign_keys=[blocked_id], backref="blocked_by_relationships"
    )
...
class Message(db.Model):
    ...
    receiver_id = db.Column(
        db.Integer, db.ForeignKey("user.id", ondelete="SET NULL"), nullable=True
    )  # Pozostawiamy dla prywatnych wiadomości
    room_id = db.Column(
    ...
    replied_to_message_id = db.Column(
        db.Integer, db.ForeignKey("message.id"), nullable=True
    )
    replied_to_message = db.relationship(
        "Message", remote_side=[id], backref="replies", uselist=False
    )
    ...
    )  # Dotyczy tylko prywatnych wiadomości, nie grup
    ...
    room = db.relationship(
        "ChatRoom", foreign_keys=[room_id], backref="messages"
    )  # Relacja do pokoju
        ...
        if self.replied_to_message:
            ...
            data["replied_to_message_preview"] = {
                ...
                "sender_username": (
                    ...
                    if self.replied_to_message.sender
                    else "Nieznany"
                ),
                "content": (
                    self.replied_to_message.content[:50] + "..."
                    if self.replied_to_message.content
                    and len(self.replied_to_message.content) > 50
                    else self.replied_to_message.content
                ),
...
class ChatRoom(db.Model):
    ...
    creator_id = db.Column(
        db.Integer, db.ForeignKey("user.id", ondelete="SET NULL"), nullable=True
    )  # Twórca pokoju, może być adminem
    created_at = db.Column(db.DateTime, default=datetime.datetime.now)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "creator_username": (
                self.creator.username if self.creator else "Nieznany"
            ),  # Dodaj username twórcy
            "created_at": self.created_at.isoformat(),
...
class RoomMembership(db.Model):
    ...
    room_id = db.Column(
        db.Integer, db.ForeignKey("chat_room.id", ondelete="CASCADE"), nullable=False
    )
    ...
    user = db.relationship(
        "User",
        backref=db.backref("room_memberships", lazy=True, cascade="all, delete-orphan"),
    )
    room = db.relationship(
        "ChatRoom",
...
def block_user_endpoint(user_to_block_id):
    ...
    blocker_id = data.get(
        "blocker_id"
    )  # Klient musi przesłać swoje ID jako blokującego
    ...
    existing_block = BlockedRelationship.query.filter_by(
        blocker_id=blocker_id, blocked_id=user_to_block_id
    ).first()
    if existing_block:
        return jsonify({"message": "User already blocked"}), 200

    try:
        new_block = BlockedRelationship(
            blocker_id=blocker_id, blocked_id=user_to_block_id
        )
...
def unblock_user_endpoint(user_to_unblock_id):
    ...
    block_to_remove = BlockedRelationship.query.filter_by(
        blocker_id=unblocker_id, blocked_id=user_to_unblock_id
    ).first()
    if not block_to_remove:
        return (
            jsonify(
                {"error": "Block relationship not found or user not blocked by you"}
            ),
            404,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "message": f"User {User.query.get(user_to_unblock_id).username} has been unblocked."
                }
            ),
            200,
        )
...
def create_room():
    ...
    try:
        ...
        membership = RoomMembership(
            user_id=creator.id, room_id=new_room.id
        )  # Użyj creator.id
        ...
        return (
            jsonify(
                {"message": "Room created successfully", "room": new_room.to_dict()}
            ),
            201,
        )
    except Exception as e:
...
def add_room_member(room_id):
    ...
    if room.password_hash:  # Jeśli pokój ma ustawione hasło
        if not password:
            return (
                jsonify({"error": "Password is required to join this room"}),
                401,
            )  # Unauthorized
        if not room.check_password(password):
            return (
                jsonify({"error": "Incorrect password for this room"}),
                401,
            )  # Unauthorized

    if RoomMembership.query.filter_by(user_id=user_id, room_id=room_id).first():
        return (
            jsonify({"message": "User is already a member of this room"}),
            200,
        )  # Już jest członkiem
...
def remove_room_member(room_id, user_id_to_leave):
    ...
    initiator_user_id = (
        int(initiator_user_id_str)
        if initiator_user_id_str and initiator_user_id_str.isdigit()
        else None
    )

    is_self_leave_action = (
        initiator_user_id is not None and initiator_user_id == user_id_to_leave
    ) or (
        admin_id_from_request is not None and admin_id_from_request == user_id_to_leave
    )

    is_admin_action_on_other = (
        admin_id_from_request is not None
        and room_to_manage.creator_id == admin_id_from_request
        and user_id_to_leave != admin_id_from_request
    )

    if not is_self_leave_action and not is_admin_action_on_other:
        if (
            admin_id_from_request is None and not is_self_leave_action
        ):  # Jeśli nie admin i nie self-leave
        ...
        # Jeśli admin_id podano, ale nie jest twórcą (a nie jest to self-leave)
            ...
            admin_id_from_request != room_to_manage.creator_id
            and not is_self_leave_action
        ):
            logger.warning(
                f"User {admin_id_from_request} (not creator) tried to remove {user_id_to_leave}."
            )
            return (
                jsonify({"error": "Only the room creator can remove other members"}),
                403,
            )
    ...
    if is_self_leave_action and user_id_to_leave == room_to_manage.creator_id:
        ...
        creator_username = (
            room_to_manage.creator.username if room_to_manage.creator else "Twórca"
        )
        ...
        try:
            ...
            db.session.delete(
                room_to_manage
            )  # To powinno usunąć pokój i kaskadowo resztę
            db.session.commit()
            ...
            return (
                jsonify(
                    {
                        "message": f"Room '{room_name_for_log}' and your membership have been deleted."
                    }
                ),
                200,
            )
        ...
        membership = RoomMembership.query.filter_by(
            user_id=user_id_to_leave, room_id=room_id
        ).first()
        if not membership:
        ...
        try:
            ...
            user_left_username = (
                user_left_obj.username if user_left_obj else f"ID {user_id_to_leave}"
            )
            action_log_message = ""
            ...
            if is_self_leave_action:  # Nie-twórca opuszcza
                ...
                admin_username = (
                    admin_obj.username if admin_obj else f"ID {admin_id_from_request}"
                )
                action_log_message = (
...
def block_user_stub(user_to_block_id):
    ...
    blocker_id_temp = request.json.get(
        "blocker_id", "NieznanyBlokujący"
    )  # Dla testów, klient może to wysłać
    logger.info(
    ...
    return (
        jsonify(
            {"message": f"User {user_to_block_id} is now on your block list (stub)."}
        ),
        200,
    )
...
@app.route(
    "/users/<int:user_to_unblock_id>/block", methods=["DELETE"]
)  # Zauważ, że używam /block z DELETE
def unblock_user_stub(user_to_unblock_id):
    ...
    return (
        jsonify(
            {
                "message": f"User {user_to_unblock_id} removed from your block list (stub)."
            }
        ),
        204,
    )  # 204 No Content jest typowe dla DELETE
...
def update_username(user_id):
    ...
    if existing_user_with_new_name and existing_user_with_new_name.id != user_id:
        return (
            jsonify({"error": "Username already taken by another user"}),
            409,
        )  # Conflict
...
def register_user():
    ...
    if EmailBlacklist.query.filter_by(email=validated_email).first():
        ...
        return (
            jsonify({"error": "This email address cannot be used for registration."}),
            403,
        )  # Forbidden
...
def download_file(server_filename):
    ...
    try:
        ...
        message_with_attachment = Message.query.filter_by(
            attachment_server_filename=server_filename
        ).first()

        original_name_to_suggest = (
            server_filename  # Domyślnie, jeśli nie znajdziemy oryginalnej
        )
        if (
            message_with_attachment
            and message_with_attachment.attachment_original_filename
        ):
            original_name_to_suggest = (
                message_with_attachment.attachment_original_filename
            )
...
def handle_typing_start(data):
    ...
    socketio.emit(
        "typing_update", {"sender_id": sender_id, "is_typing": True}, room=receiver_id
    )
    logger.debug(
...
def handle_edit_message(data):
    ...
    if message_id is None or new_content is None or editor_user_id is None:
        ...
        emit(
            "message_edit_failed",
            {"message_id": message_id, "error": "Niekompletne żądanie."},
            room=request.sid,
        )
        return
    ...
    if not message_to_edit:
        ...
        emit(
            "message_edit_failed",
            {"message_id": message_id, "error": "Wiadomość nie istnieje."},
            room=request.sid,
        )
        return
    ...
    if not new_content.strip():
        ...
        emit(
            "message_edit_failed",
            {"message_id": message_id, "error": "Treść wiadomości nie może być pusta."},
            room=request.sid,
        )
        return
    ...
    if len(new_content) > 5000:  # Limit długości treści
        ...
        emit(
            "message_edit_failed",
            {"message_id": message_id, "error": "Treść wiadomości jest zbyt długa."},
            room=request.sid,
        )
        return
    ...
    try:
        ...
        if (
            message_to_edit.receiver_id == editor_user_id
        ):  # Jeśli edytował odbiorca (rzadko)
...
def handle_delete_message(data):
    ...
    deleter_user_id = data.get(
        "deleter_user_id"
    )  # ID użytkownika, który kliknął "Usuń"
    ...
    original_room_id = (
        message_to_delete.room_id
    )  # Zapamiętaj ID pokoju, jeśli to wiadomość grupowa

    try:
        db.session.delete(message_to_delete)
        db.session.commit()
        logger.info(
            f"Delete message: Wiadomość o ID {message_id} została usunięta z bazy przez użytkownika {deleter_user_id}."
        )
        ...
        if original_room_id:  # Jeśli to była wiadomość grupowa
            ...
            logger.debug(
                f"Delete message: Informowanie członków pokoju {original_room_id} o usunięciu wiadomości {message_id}."
            )
            socketio.emit(
            ...
            original_receiver_id and original_receiver_id != deleter_user_id
        ):  # Wiadomość prywatna (logika bez zmian)
            other_participant_id = original_receiver_id
            logger.debug(
                f"Delete message: Informowanie drugiego uczestnika (ID: {other_participant_id}) o usunięciu wiadomości {message_id}."
            )
            socketio.emit(
                "message_deleted_successfully",
                payload_success,
        ...
        db.session.rollback()
        logger.error(
            f"Delete message: Błąd bazy danych podczas usuwania wiadomości ID {message_id}: {e_db_delete}"
        )
        emit(
            "message_delete_failed",
            {
...
def handle_mark_as_read(data):
    ...
    conversation_partner_id = data.get(
        "conversation_partner_id"
    )  # Alternatywnie, ID drugiej osoby w czacie
    ...
    if message_ids_to_mark and isinstance(message_ids_to_mark, list):
        ...
        conversation_partner_id is not None
    ):  # Oznacz wszystkie od tego partnera dla tego czytelnika
        messages_to_update = Message.query.filter(
    ...
    if not messages_to_update:
        logger.debug(
            f"Mark as read: Brak wiadomości do oznaczenia jako przeczytane dla czytelnika {reader_user_id}."
        )
        return
    ...
    for msg in messages_to_update:
        ...
        if msg.sender_id != reader_user_id:  # Nie wysyłaj do siebie samego
            logger.debug(
                f"Mark as read: Wysyłanie 'message_read_update' do nadawcy {msg.sender_id} dla wiadomości {msg.id}"
            )
    ...
    if updated_message_ids_for_sender_notification:
        try:
            db.session.commit()
            logger.info(
                f"Mark as read: Oznaczono {len(updated_message_ids_for_sender_notification)} wiadomości jako przeczytane przez użytkownika {reader_user_id}."
            )
            ...
            db.session.rollback()
            logger.error(
                f"Mark as read: Błąd bazy danych przy oznaczaniu wiadomości jako przeczytanych: {e_db_commit}"
            )
...
def handle_typing_stop(data):
    ...
    # Emituj do odbiorcy (do jego roomu)
    socketio.emit(
        "typing_update", {"sender_id": sender_id, "is_typing": False}, room=receiver_id
    )
    logger.debug(
        f"User {sender.username} (ID: {sender_id}) stopped typing to {receiver_id}."
    )


@socketio.on(
    "send_message"
)  # <-- ZMIANA NAZWY EVENTU, KLIENT TEŻ BĘDZIE MUSIAŁ EMITOWAĆ 'send_message'
def handle_send_message(data):  # <-- ZMIANA NAZWY FUNKCJI
    ...
    logger.debug(
        f"[handle_send_message] Received data: {data} from SID {request.sid}"
    ...
    if replied_to_message_id:
        ...
        if not quoted_msg:
            logger.warning(
                f"[handle_send_message] Message send failed: Replied-to message ID {replied_to_message_id} not found."
            )
            emit(
                "message_error",
                {
                    "message": f"Nie można odpowiedzieć na wiadomość: oryginalna wiadomość (ID: {replied_to_message_id}) nie istnieje."
                },
                room=request.sid,
            )
            return
    ...
    logger.debug(
        f"[handle_send_message] Processing with Sender ID: {sender_id}, Receiver ID: {receiver_id}, Room ID: {room_id}"
    )

    if not sender:  # Najpierw sprawdźmy nadawcę
        logger.warning(
            f"[handle_send_message] Message send failed: Invalid sender_id ({sender_id})"
        )
    ...
    if (
        room_id is not None
    ):  # Jeśli room_id jest obecne, traktujemy jako wiadomość grupową
        room = ChatRoom.query.get(room_id)
        if not room:
            logger.warning(
                f"[handle_send_message] Message send failed: Invalid room_id ({room_id}) for group message."
            )
            emit(
                "message_error",
                {"message": "Invalid room for group message"},
                room=request.sid,
            )
            return
        # Weryfikacja członkostwa dla wiadomości grupowej
        if not RoomMembership.query.filter_by(
            user_id=sender_id, room_id=room.id
        ).first():
            logger.warning(
                f"[handle_send_message] Message send failed: Sender {sender_id} is not a member of room {room.id}."
            )
            emit(
                "message_error",
            ...
            return
        logger.debug(
            f"[handle_send_message] Determined message type: GROUP (Room: {room.name})"
        )
    elif (
        receiver_id is not None
    ):  # Jeśli room_id jest None, ale receiver_id jest, to wiadomość prywatna
        receiver = User.query.get(receiver_id)
        if not receiver:
            logger.warning(
                f"[handle_send_message] Message send failed: Invalid receiver_id ({receiver_id}) for private message."
            )
            emit(
                "message_error",
                {"message": "Invalid receiver for private message"},
                room=request.sid,
            )
            return
        # Sprawdź, czy ODBIORCA (receiver) zablokował NADAWCĘ (sender)
        is_sender_blocked_by_receiver = BlockedRelationship.query.filter_by(
            blocker_id=receiver.id, blocked_id=sender.id
        ).first()

        if is_sender_blocked_by_receiver:
            logger.warning(
                f"[handle_send_message] Message send failed (DM): Sender {sender.id} ({sender.username}) is blocked by receiver {receiver.id} ({receiver.username})."
            )
            emit(
                "message_error",
                {
                    "message": f"Nie możesz wysłać wiadomości do {receiver.username}, ponieważ zablokował(a) Cię."
                },
                room=request.sid,
            )
            return
        ...
        is_receiver_blocked_by_sender = BlockedRelationship.query.filter_by(
            blocker_id=sender.id, blocked_id=receiver.id
        ).first()
        if is_receiver_blocked_by_sender:
            logger.warning(
                f"[handle_send_message] Message send failed (DM): Sender {sender.id} ({sender.username}) has blocked receiver {receiver.id} ({receiver.username}). Client should handle this."
            )
            emit(
                "message_error",
                {
                    "message": f"Zablokowałeś użytkownika {receiver.username}. Odblokuj, aby wysłać wiadomość."
                },
                room=request.sid,
            )
            return
        ...
        if is_sender_blocked_by_receiver:
            logger.warning(
                f"[handle_send_message] Message send failed: Sender {sender.id} ({sender.username}) is blocked by receiver {receiver.id} ({receiver.username})."
            )
            emit(
                "message_error",
                {
        ...
        if is_receiver_blocked_by_sender:
            logger.warning(
                f"[handle_send_message] Message send failed: Sender {sender.id} ({sender.username}) has blocked receiver {receiver.id} ({receiver.username}). Client should have prevented this."
            )
        ...
        logger.debug(
            f"[handle_send_message] Determined message type: PRIVATE (Receiver: {receiver.username})"
        )
    else:  # Ani room_id ani receiver_id
        logger.warning(
            f"[handle_send_message] Message send failed: Neither receiver_id nor room_id provided by sender {sender_id}."
        )
        emit(
            "message_error",
            {"message": "Message must have a receiver or a room"},
            room=request.sid,
        )
        return
    ...
    if not content and not attachment_server_filename:
        logger.warning(
            f"[handle_send_message] Message send failed: Empty content and no attachment from sender {sender_id}."
        )
        emit(
            "message_error",
            {"message": "Message content cannot be empty without an attachment."},
            room=request.sid,
        )
        return
    ...
    if content and len(content) > 5000:
        logger.warning(
            f"[handle_send_message] Message send failed: Content too long from sender {sender_id}."
        )
        emit(
            "message_error",
    ...
    logger.debug(
        f"[handle_send_message] Message saved to DB. ID: {new_message.id}. Prepared msg_data: {msg_data}"
    )
    ...
    if receiver:  # Wiadomość prywatna
        log_msg_prefix = (
            f"PRIVATE message from {sender.username} to {receiver.username}:"
        )
        logger.info(
            f"[EMIT] Sending 'private_message_sent' to sender's SID: {request.sid}"
        )
        emit("private_message_sent", msg_data, room=request.sid)

        logger.info(
            f"[EMIT] Sending 'private_message_received' to receiver's room (user ID): {receiver.id}"
        )
        ...
        logger.info(
            f"[EMIT] Sending 'group_message_sent' to sender's SID: {request.sid} for room {room.id}"
        )
        emit("group_message_sent", msg_data, room=request.sid)

        logger.info(
            f"[EMIT] Sending 'group_message_received' to Socket.IO room_id: {room.id}, skipping sender's SID: {request.sid}"
        )
        socketio.emit(
            "group_message_received", msg_data, room=room.id, skip_sid=request.sid
        )
    else:
        logger.error(
            "[handle_send_message] CRITICAL: No receiver AND no room. This should not happen!"
        )
        return  # Nie emituj nic, jeśli doszło do tego stanu

    final_log_message = log_msg_prefix
    if content:
        final_log_message += f" '{content[:50]}...'"
    if attachment_original_filename:
        final_log_message += f" [Attachment: {attachment_original_filename}]"
    if replied_to_message_id:
        final_log_message += f" (Replied to: {replied_to_message_id})"
