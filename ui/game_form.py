...
from launcher.utils import IMAGES_FOLDER, load_photoimage_from_path
...
class GameForm:
    def __init__(self, parent, title, game_name="", game_data=None):
        ...
        self._current_profiles = (
            game_data.get("launch_profiles", [])
            if game_data
            else [{"name": "Default", "exe_path": None, "arguments": ""}]
        )
        # Zapewnij istnienie domyślnego profilu
        if not any(
            p.get("name", "").lower() == "default" for p in self._current_profiles
        ):
            self._current_profiles.insert(
        ...
        self.game_type_var = tk.StringVar(
            value=self.game_data.get("game_type", "pc")
        )  # Domyślnie "pc"
        self.game_type_var.trace_add("write", self._on_game_type_change)  # Śledź zmiany

        ttk.Label(self.top, text="Typ gry:").grid(
        ...
        ttk.Label(self.top, text="Nazwa gry:").grid(
            row=1, column=0, padx=10, pady=5, sticky="w"
        )
        self.name_entry = ttk.Entry(self.top)
        self.name_entry.grid(
            row=1, column=1, columnspan=2, padx=10, pady=5, sticky="ew"
        )
        ...
        self.pc_exe_btn = ttk.Button(
            self.top, text="Wybierz...", command=self.select_exe
        )
        self.pc_exe_btn.grid(row=2, column=2, padx=5, pady=5)
        self.pc_game_widgets.extend(
            [self.pc_exe_label, self.exe_entry, self.pc_exe_btn]
        )
        ...
        self.emulator_game_widgets.extend(
            [self.rom_label, self.rom_entry, self.rom_btn]
        )
        ...
        ttk.Label(self.top, text="Ścieżka zapisów:").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="w"
        )
        current_form_row += 1
        self.save_entry = ttk.Entry(self.top)
        self.save_entry.grid(
            row=current_form_row - 1, column=1, padx=10, pady=5, sticky="ew"
        )  # Użyj poprzedniego wiersza
        ...
        ttk.Label(self.top, text="Okładka:").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="w"
        )
        current_form_row += 1
        ...
        # Gatunki
        ttk.Label(self.top, text="Gatunki:").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="nw"
        )
        current_form_row += 1
        genres_frame = ttk.Frame(self.top)
        genres_frame.grid(
            row=current_form_row - 1, column=1, padx=10, pady=5, sticky="nsew"
        )  # Użyj poprzedniego wiersza
        ...
        genres_scrollbar = ttk.Scrollbar(
            genres_frame, orient="vertical", command=self.genres_listbox.yview
        )
        genres_scrollbar.grid(row=0, column=1, sticky="ns")
        self.genres_listbox.config(yscrollcommand=genres_scrollbar.set)
        genres_btn = ttk.Button(
            self.top, text="Zarządzaj Gatunkami", command=self.parent.manage_genres
        )
        ...
        ttk.Label(self.top, text="Ocena (1-10):").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="w"
        )
        current_form_row += 1
        self.rating_entry = ttk.Entry(self.top)
        self.rating_entry.grid(
            row=current_form_row - 1,
            column=1,
            columnspan=2,
            padx=10,
            pady=5,
        ...
        ttk.Label(self.top, text="Wersja gry:").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="w"
        )
        current_form_row += 1
        ...
        ttk.Label(self.top, text="Tagi (oddzielone przecinkami):").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="w"
        )
        current_form_row += 1
        ...
        ttk.Label(self.top, text="Notatki:").grid(
            row=current_form_row, column=0, padx=10, pady=5, sticky="nw"
        )
        current_form_row += 1
        ...
        notes_scrollbar = ttk.Scrollbar(
            self.top, orient="vertical", command=self.notes_text.yview
        )
        notes_scrollbar.grid(
            row=current_form_row - 1, column=3, padx=(0, 10), pady=5, sticky="ns"
        )
        self.notes_text.config(yscrollcommand=notes_scrollbar.set)

        # Profile Uruchomieniowe (ukryte dla gier emulowanych)
        self.pc_profiles_frame = ttk.LabelFrame(
            self.top, text=" Profile Uruchomieniowe (PC) ", padding=(10, 5)
        )
        self.pc_profiles_frame.grid(
            row=current_form_row,
            column=0,
            columnspan=4,
            sticky="nsew",
            padx=10,
        ...
        profile_tree_frame = ttk.Frame(self.pc_profiles_frame)
        profile_tree_frame.grid(row=0, column=0, columnspan=2, sticky="nsew", pady=5)
        profile_tree_frame.columnconfigure(0, weight=1)
        profile_tree_frame.rowconfigure(0, weight=1)
        profile_columns = ("Profil", "Argumenty", "Spec. EXE")
        ...
        self.profile_tree.heading("Profil", text="Nazwa Profilu")
        self.profile_tree.heading("Argumenty", text="Argumenty")
        self.profile_tree.heading("Spec. EXE", text="Ścieżka EXE (jeśli inna)")
        self.profile_tree.column("Profil", width=120, anchor=tk.W)
        self.profile_tree.column("Argumenty", width=150, anchor=tk.W)
        self.profile_tree.column("Spec. EXE", width=180, anchor=tk.W)
        profile_scrollbar = ttk.Scrollbar(
            profile_tree_frame, orient="vertical", command=self.profile_tree.yview
        )
        ...
        profile_buttons_frame = ttk.Frame(self.pc_profiles_frame)
        profile_buttons_frame.grid(row=1, column=0, columnspan=2, pady=5)
        ttk.Button(
            profile_buttons_frame, text="Dodaj Profil", command=self.add_edit_profile
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            profile_buttons_frame,
            text="Edytuj Zaznaczony",
            command=lambda: self.add_edit_profile(edit_mode=True),
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            profile_buttons_frame, text="Usuń Zaznaczony", command=self.delete_profile
        ).pack(side=tk.LEFT, padx=5)
        current_form_row += 1
        ...
        button_frame.grid(
            row=current_form_row, column=0, columnspan=4, pady=15, sticky="e"
        )  # Użyj ostatniego wiersza
        # ... (przyciski Parsuj, Zapisz, Anuluj) ...
        parse_nfo_btn = ttk.Button(
            button_frame, text="Parsuj .NFO", command=self.parse_nfo_action
        )
        parse_nfo_btn.pack(side=tk.LEFT, padx=5)
        parse_folder_btn = ttk.Button(
            button_frame, text="Parsuj Nazwę Fold.", command=self.parse_folder_action
        )
        parse_folder_btn.pack(side=tk.LEFT, padx=5)
        save_btn = ttk.Button(button_frame, text="Zapisz", command=self.save)
        save_btn.pack(side=tk.LEFT, padx=5)
        cancel_btn = ttk.Button(button_frame, text="Anuluj", command=self.top.destroy)
        cancel_btn.pack(side=tk.LEFT, padx=5)

        # Konfiguracja rozciągania
        self.top.columnconfigure(1, weight=1)
        self.top.rowconfigure(
            current_form_row - 1, weight=1
        )  # Ostatni wiersz z zawartością (np. Profile lub Notatki) się rozciąga
        ...
        if self.game_data:
            ...
            self.rating_entry.insert(
                0, str(rating_value) if rating_value is not None else ""
            )
                ...
                for genre in self.game_data["genres"]:
                    if genre in self.all_genres:
                        try:
                            ...
                            self.genres_listbox.selection_set(index)
                        except ValueError:
                            logging.warning(
                                f"Gatunek '{genre}' z danych gry nie znaleziony w liście."
                            )
    ...
    def select_rom(self):
        ...
        filetypes = [
            (
                ...
                "*.iso *.bin *.cue *.gcm *.rvz *.wbfs *.wad *.nca *.nsp *.xci *.rom *.nes *.sfc *.smc *.md *.gen *.sms *.gg *.pce *.sgx *.zip *.7z",
            ),
        ...
        path = filedialog.askopenfilename(
            title="Wybierz plik ROM/ISO gry", filetypes=filetypes
        )
        if path:
    ...
    def update_emulator_list(self):
        ...
        if hasattr(self, "emulator_combo") and self.emulator_combo.winfo_exists():
            ...
            if current_emu not in emulator_names:
                self.emulator_var.set(
                    ""
                )  # Wyczyść wybór, jeśli emulator został usunięty
    ...
    def parse_nfo_action(self):
        ...
        if not self.game_data or not self.game_data.get("exe_path"):
            messagebox.showwarning(
                "Brak informacji",
                "Nie można parsować .nfo bez ścieżki do pliku .exe gry.",
            )
            return
        ...
        nfo_data = self.parent.parse_nfo_file(
            game_folder
        )  # Wywołujemy funkcję z głównej klasy
        logging.info(f"Otrzymano dane z NFO w formularzu: {nfo_data}")  # Dodaj ten log
        if not nfo_data:
            messagebox.showinfo(
                "Informacja",
                "Nie znaleziono pliku .nfo lub nie udało się sparsować danych.",
            )
            return
        ...
        if (
            nfo_data.get("title") and not self.name_entry.get()
        ):  # Nazwy nie zmieniamy jeśli jest edycja
            pass  # self.name_entry.insert(0, nfo_data["title"])
        if (
            nfo_data.get("release_date") and not self.version_entry.get()
        ):  # Użyjmy pola wersji dla daty z NFO
    ...
    def parse_folder_action(self):
        ...
        if not self.game_data or not self.game_data.get("exe_path"):
            messagebox.showwarning(
                "Brak informacji",
                "Nie można parsować nazwy folderu bez ścieżki do pliku .exe gry.",
            )
            return
        ...
        folder_data = self.parent.parse_folder_name_metadata(
            folder_name
        )  # Wywołujemy funkcję z głównej klasy
        ...
        if not folder_data:
            messagebox.showinfo(
                "Informacja",
                "Nie udało się sparsować dodatkowych danych z nazwy folderu.",
            )
            return

        # Aktualizuj pola formularza
        if (
            folder_data.get("year") and not self.version_entry.get()
        ):  # Użyjmy pola wersji dla roku z folderu
            self.version_entry.delete(0, tk.END)
            self.version_entry.insert(0, f"Year: {folder_data['year']}")
        if folder_data.get("publisher"):
            self.notes_text.insert(
                "1.0", f"Folder Publisher: {folder_data['publisher']}\n"
            )
    ...
    def save(self):
        ...
        if self.game_data and "name" in self.game_data:  # Edycja
            ...
            if not name_key:
                messagebox.showwarning(
                    "Błąd", "Nazwa gry jest wymagana.", parent=self.top
                )
                return
            if name_key in self.parent.games:
                messagebox.showerror(
                    "Błąd", f"Gra o nazwie '{name_key}' już istnieje.", parent=self.top
                )
                return
        ...
        if game_type == "pc":
            ...
            if not exe_path:
                messagebox.showwarning(
                    "Błąd",
                    "Ścieżka do pliku wykonywalnego (PC) jest wymagana.",
                    parent=self.top,
                )
                return
            if not os.path.exists(exe_path):  # Sprawdź istnienie dla PC
                messagebox.showerror(
                    "Błąd",
                    f"Plik wykonywalny PC nie istnieje:\n{exe_path}",
                    parent=self.top,
                )
                return
            game_data_to_save["exe_path"] = exe_path
            game_data_to_save["launch_profiles"] = (
                self._current_profiles
            )  # Zapisz profile tylko dla PC
            ...
            profile_name_for_log = (
                emulator_name  # Użyj nazwy emulatora jako informacji o "profilu"
            )
            if not emulator_name:
                messagebox.showwarning(
                    "Błąd", "Wybierz emulator dla gry emulowanej.", parent=self.top
                )
                return
            if not rom_path:
                messagebox.showwarning(
                    "Błąd", "Ścieżka do pliku ROM/ISO jest wymagana.", parent=self.top
                )
                return
            if not os.path.exists(rom_path):  # Sprawdź istnienie ROMu
                messagebox.showerror(
                    "Błąd", f"Plik ROM/ISO nie istnieje:\n{rom_path}", parent=self.top
                )
                return
            ...
            if emulator_name not in self.parent.config.get("emulators", {}):
                messagebox.showerror(
                    "Błąd",
                    f"Wybrany emulator '{emulator_name}' nie jest skonfigurowany w Ustawieniach.",
                    parent=self.top,
                )
                return
            ...
            game_data_to_save["launch_profiles"] = (
                []
            )  # Gry emulowane nie mają profili w ten sam sposób
        ...
        if cover_path_from_entry:
            if os.path.exists(cover_path_from_entry):
                ...
                if os.path.normcase(
                    os.path.abspath(cover_path_from_entry)
                ) != os.path.normcase(os.path.abspath(original_cover_path or "")):
                    cover_changed = True
                final_cover_image = cover_path_from_entry  # Zapisz ścieżkę z pola
            else:
                messagebox.showerror(
                    "Błąd Ścieżki",
                    f"Podana ścieżka do okładki nie istnieje:\n{cover_path_from_entry}",
                    parent=self.top,
                )
                return
        ...
        try:
            ...
            messagebox.showwarning(
                "Błąd", "Ocena musi być liczbą (np. 7.5) lub pusta.", parent=self.top
            )
            return

        game_data_to_save["version"] = self.version_entry.get().strip()
        tags_str = self.tags_entry.get().strip()
        game_data_to_save["tags"] = [
        ...
        if cover_changed:
            try:
                ...
                logging.info(
                    "Wyczyszczono cache load_photoimage_from_path z powodu zmiany okładki."
                )
    ...
    def refresh_profile_treeview(self):
        ...
        for idx, profile in enumerate(self._current_profiles):
            ...
            exe = (
                profile.get("exe_path", "") or "(Domyślny)"
            )  # Pokaż "Domyślny", jeśli exe_path to None lub ""
    ...
    def add_edit_profile(self, edit_mode=False):
        ...
        if edit_mode:
            ...
            if not selection:
                messagebox.showwarning(
                    "Brak zaznaczenia", "Zaznacz profil, który chcesz edytować."
                )
                return
            ...
            try:
                ...
                messagebox.showerror(
                    "Błąd", "Nie można odczytać danych wybranego profilu."
                )
                return
        else:
        ...
        # Pola do edycji
        ttk.Label(profile_window, text="Nazwa profilu:").grid(
            row=0, column=0, padx=10, pady=5, sticky="w"
        )
        ...
        ttk.Label(profile_window, text="Ścieżka .exe (opcjonalna):").grid(
            row=1, column=0, padx=10, pady=5, sticky="w"
        )
        ...
        exe_btn = ttk.Button(
            profile_window,
            text="Wybierz...",
            command=lambda v=exe_var: self.select_profile_exe(v),
        )
        exe_btn.grid(row=1, column=2, padx=5, pady=5)

        ttk.Label(profile_window, text="Argumenty linii komend:").grid(
        ...
        def save_profile_data():
            ...
            for i, prof in enumerate(self._current_profiles):
                ...
                if prof.get("name", "").lower() == new_name.lower():
                    messagebox.showwarning(
                        "Nazwa zajęta",
                        f"Profil o nazwie '{new_name}' już istnieje.",
                        parent=profile_window,
                    )
                    return
        ...
        cancel_button = ttk.Button(
            btn_frame, text="Anuluj", command=profile_window.destroy
        )
    ...
    def delete_profile(self):
        ...
        if not selection:
            messagebox.showwarning(
                "Brak zaznaczenia", "Zaznacz profil, który chcesz usunąć."
            )
            return
        selected_iid = selection[0]
        try:
            profile_index = int(selected_iid)
            profile_name = self._current_profiles[profile_index].get(
                "name", f"Profil {profile_index+1}"
            )

            # Nie pozwól usunąć ostatniego profilu ani profilu "Default", jeśli jest jedyny
            if len(self._current_profiles) <= 1:
                messagebox.showerror(
                    "Błąd", "Nie można usunąć ostatniego profilu.", parent=self.top
                )
                return
            if profile_name.lower() == "default" and len(self._current_profiles) == 1:
                messagebox.showerror(
                    "Błąd",
                    "Nie można usunąć profilu 'Default', gdy jest jedyny.",
                    parent=self.top,
                )
                return
            ...
            messagebox.showerror(
                "Błąd", "Nie można zidentyfikować wybranego profilu.", parent=self.top
            )
