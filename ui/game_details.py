...
from launcher.utils import IMAGES_FOLDER, RESAMPLING, load_photoimage_from_path, save_config
...
class GameDetailsWindow(tk.Toplevel):
    def __init__(self, parent, launcher_instance, game_name):
        ...
        self.game_data = self.launcher.games.setdefault(
            game_name, {}
        )  # Pobierz lub utwórz pusty słownik
        self.game_data.setdefault(
            "screenshots", []
        )  # Upewnij się, że klucz checklist istnieje
        self.game_data.setdefault(
            "autoscan_screenshots", []
        )  # Upewnij się, że klucz autoscan istnieje
        self.game_data.setdefault(
            "checklist", []
        )  # Upewnij się, że klucz checklist istnieje
        self.game_data.setdefault(
        ...
        if not self.game_data:
            messagebox.showerror(
                "Błąd", f"Nie znaleziono danych dla gry: {game_name}", parent=self
            )
            self.destroy()
            return
        ...
        cover_frame.grid(
            row=0, column=0, rowspan=3, sticky="nsew", padx=(0, 10)
        )  # rowspan=3 by objąć statystyki
        ...
        self.release_label = ttk.Label(
            basic_info_frame, text="Data wydania: -", font=("Segoe UI", 9)
        )
        self.release_label.grid(row=0, column=0, sticky="w", padx=(0, 5))
        self.developer_label = ttk.Label(
            basic_info_frame, text="Deweloper: -", font=("Segoe UI", 9), wraplength=180
        )
        self.developer_label.grid(row=0, column=2, sticky="w", padx=(10, 5))
        self.publisher_label = ttk.Label(
            basic_info_frame, text="Wydawca: -", font=("Segoe UI", 9), wraplength=180
        )
        self.publisher_label.grid(row=1, column=2, sticky="w", padx=(10, 5))
        self.website_button = ttk.Button(
            basic_info_frame,
            text="Strona WWW",
            style="Link.TButton",
            state=tk.DISABLED,
        ...
        self.notebook.grid(
            row=1, column=1, sticky="nsew", pady=5
        )  # Zajmuje główną przestrzeń
        ...
        self.notes_text = tk.Text(
            desc_notes_frame, wrap=tk.WORD, height=8, relief=tk.FLAT, state=tk.DISABLED
        )
        ...
        notes_scroll = ttk.Scrollbar(
            desc_notes_frame, orient="vertical", command=self.notes_text.yview
        )
        ...
        self.edit_notes_button = ttk.Button(
            desc_notes_frame, text="Edytuj Notatki", command=self.toggle_notes_edit
        )
        self.edit_notes_button.grid(
        ...
        self.genres_api_label = ttk.Label(
            api_info_frame,
            text="Gatunki (API): -",
            wraplength=500,
            justify=tk.LEFT,
        ...
        req_scroll = ttk.Scrollbar(
            req_info_frame, orient="vertical", command=self.req_text.yview
        )
        ...
        self.screenshots_frame.columnconfigure(
            1, weight=1
        )  # Daj miejsce na przycisk skanowania
        ...
        ttk.Button(
            ss_top_button_frame,
            text="Dodaj Screenshoty...",
            command=self.add_screenshots,
        ...
        self.screenshot_canvas = tk.Canvas(
            self.screenshots_frame, bg="#1e1e1e", highlightthickness=0
        )
        ss_scrollbar = ttk.Scrollbar(
            self.screenshots_frame,
            orient="vertical",
            command=self.screenshot_canvas.yview,
        )
        self.screenshot_inner_frame = ttk.Frame(self.screenshot_canvas)
        self.screenshot_canvas_window_id = self.screenshot_canvas.create_window(
            (0, 0), window=self.screenshot_inner_frame, anchor="nw"
        )
        self.screenshot_canvas.configure(yscrollcommand=ss_scrollbar.set)
        self.screenshot_canvas.grid(
            row=1, column=0, sticky="nsew"
        )  # Zostaje w kolumnie 0
        ss_scrollbar.grid(row=1, column=1, sticky="ns")  # Scrollbar w kolumnie 1
        self.screenshot_canvas.bind(
            "<Configure>",
            lambda e: self.screenshot_canvas.itemconfig(
                self.screenshot_canvas_window_id, width=e.width
            ),
        )
        self.screenshot_inner_frame.bind(
            "<Configure>",
            lambda e: self.screenshot_canvas.configure(
                scrollregion=self.screenshot_canvas.bbox("all")
            ),
        )
        self.screenshot_canvas.bind_all(
            "<MouseWheel>", self._on_screenshot_mousewheel, add="+"
        )  # Zmieniono bind na bind_all z add='+'

        self.checklist_tab_frame = ttk.Frame(self.notebook, padding=10)
        self.notebook.add(self.checklist_tab_frame, text="Checklista")
        self.checklist_tab_frame.columnconfigure(
            0, weight=1
        )  # Kolumna z Treeview rośnie
        ...
        self.checklist_tree.column(
            "status", width=40, stretch=False, anchor="center"
        )  # Wąska kolumna dla statusu
        self.checklist_tree.column(
            "task", width=400, anchor="w"
        )  # Główna kolumna na opis
        ...
        self.checklist_tree.bind(
            "<Double-1>", self._toggle_task_status_event
        )  # Podwójne kliknięcie przełącza
        ...
        checklist_buttons_frame.grid(
            row=1, column=0, columnspan=2, pady=(10, 0), sticky="ew"
        )

        ttk.Button(
            checklist_buttons_frame, text="Dodaj Zadanie", command=self._add_task
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            checklist_buttons_frame, text="Edytuj Zaznaczone", command=self._edit_task
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            checklist_buttons_frame, text="Usuń Zaznaczone", command=self._delete_task
        ).pack(side=tk.LEFT, padx=5)

        actions_tab_outer_frame = ttk.Frame(
            self.notebook
        )  # Ramka zewnętrzna dla Canvas i Scrollbar
        ...
        actions_canvas = tk.Canvas(
            actions_tab_outer_frame, bg="#1e1e1e", highlightthickness=0
        )
        actions_canvas.grid(row=0, column=0, sticky="nsew")

        # Scrollbar
        actions_scrollbar = ttk.Scrollbar(
            actions_tab_outer_frame, orient="vertical", command=actions_canvas.yview
        )
        ...
        scrollable_actions_frame_window_id = actions_canvas.create_window(
            (0, 0), window=scrollable_actions_frame, anchor="nw"
        )

        # Bindowanie resize i scroll
        def _configure_actions_canvas(event):
            if (
                actions_canvas.winfo_exists()
                and scrollable_actions_frame.winfo_exists()
            ):
                canvas_width = event.width
                actions_canvas.itemconfig(
                    scrollable_actions_frame_window_id, width=canvas_width
                )
        ...
        scrollable_actions_frame.bind(
            ...
            lambda e: (
                actions_canvas.configure(scrollregion=actions_canvas.bbox("all"))
                if actions_canvas.winfo_exists()
                else None
            ),
        )
        ...
        def _on_actions_mousewheel(event):
            ...
            while curr is not None:
                if curr == actions_canvas:
                    is_actions_area = True
                    break
                if curr == self:
                    break  # Ogranicz do okna szczegółów
                try:
                    curr = curr.master
                except tk.TclError:
                    break
        ...
        actions_canvas.bind_all(
            "<MouseWheel>", _on_actions_mousewheel, add="+"
        )  # Dodaj, ale pozwól też innym działać

        # --- Umieść LabelFrame'y WEWNĄTRZ scrollable_actions_frame ---
        scrollable_actions_frame.columnconfigure(
            0, weight=1
        )  # Pozwól LabelFrame'om wypełnić szerokość

        # Sekcja: Główne Akcje
        main_actions_frame = ttk.LabelFrame(
            scrollable_actions_frame, text=" Zarządzanie Grą ", padding=(10, 5)
        )
        ...
        ttk.Button(
            main_actions_frame,
            text="Edytuj Dane Gry...",
            command=lambda: self.launcher.edit_game(self.game_name),
        ).pack(pady=3, anchor="w", fill="x")
        ttk.Button(
            main_actions_frame,
            text="Zarządzaj Zapisami...",
            command=lambda: self.launcher.manage_saves(self.game_name),
        ).pack(pady=3, anchor="w", fill="x")
        ttk.Button(
            main_actions_frame,
            text="Przejdź do Menedżera Modów",
            command=self.show_mods_for_game,
        ).pack(pady=3, anchor="w", fill="x")
        ttk.Button(
            main_actions_frame,
            text="Resetuj Statystyki...",
            command=lambda: self.launcher.reset_stats(self.game_name),
        ).pack(pady=3, anchor="w", fill="x")

        # Sekcja: Zarządzanie Okładką
        cover_actions_frame = ttk.LabelFrame(
            scrollable_actions_frame, text=" Zarządzanie Okładką ", padding=(10, 5)
        )
        cover_actions_frame.grid(row=1, column=0, sticky="ew", padx=5, pady=5)
        ttk.Button(
            cover_actions_frame,
            text="Ustaw Własną Okładkę...",
            command=self.select_custom_cover,
        ).pack(pady=3, anchor="w", fill="x")
        ttk.Button(
            cover_actions_frame,
            text="Pobierz/Użyj Okładki z RAWG",
            command=lambda: self.fetch_cover_from_rawg(force=True),
        ).pack(pady=3, anchor="w", fill="x")
        ttk.Button(
            cover_actions_frame,
            text="Usuń Okładkę (przywróć domyślną)",
            command=self.remove_cover,
        ).pack(pady=3, anchor="w", fill="x")

        # Ramka Statystyk Launchera (bez zmian)
        self.launcher_stats_frame = ttk.Frame(
            main_frame
        )  # Zmieniono parent na main_frame
        self.launcher_stats_frame.grid(
        ...
        bottom_action_frame.grid(
            row=3, column=1, sticky="ew", pady=(10, 0)
        )  # Zmieniono parent na main_frame
        bottom_action_frame.columnconfigure(
            (0, 1, 2), weight=1
        )  # Rozłóż przyciski równomiernie

        self.fetch_api_button = ttk.Button(
            bottom_action_frame,
            text="Pobierz Dane z RAWG",
            command=self.start_fetch_details_thread,
        )
        self.fetch_api_button.grid(row=0, column=0, padx=5, pady=2, sticky="ew")

        # Przycisk Uruchom (w środku)
        self.create_launch_button_with_profiles(
            bottom_action_frame, start_column=1
        )  # Przekaż ramkę i kolumnę

        close_btn = ttk.Button(
            bottom_action_frame, text="Zamknij", command=self.destroy
        )
    ...
    def start_scan_for_this_game(self):
        ...
        logging.info(
            f"Ręczne uruchomienie skanowania screenshotów dla gry: {self.game_name}"
        )
    ...
    def select_custom_cover(self):
        ...
        if path and os.path.exists(path):
            ...
            norm_original_abs = (
                os.path.normcase(os.path.abspath(original_cover_path))
            ...
            if norm_source_abs != norm_original_abs or not original_cover_path:
                ...
                try:
                    ...
                    if (
                        original_cover_path
                        and os.path.exists(original_cover_path)
                        and norm_original_abs != norm_dest_abs
                    ):
                        try:
                            os.remove(original_cover_path)
                        except OSError as e:
                            logging.error(
                                f"Błąd usuwania starej okładki {original_cover_path}: {e}"
                            )
                    ...
                    self.launcher.root.after(
                        150,
                        lambda gn=self.game_name: self.launcher._force_refresh_tile(gn),
                    )
                except Exception as e:
                    messagebox.showerror(
                        "Błąd", f"Nie udało się ustawić nowej okładki: {e}", parent=self
                    )
                    logging.exception(
                        f"Błąd przy ustawianiu własnej okładki dla {self.game_name}"
                    )
    ...
    def load_cover_image(self, size=(300, 450)):
        ...
        if self.cover_photo:
            ...
            logging.warning(
                f"Nie można załadować dużej okładki dla {self.game_name} ({original_path}). Używanie placeholdera tekstowego."
            )
            self.cover_label.config(
                image=None, text="Brak Okładki"
            )  # Użyj tekstu, jeśli ładowanie zawiedzie

    def create_stats_labels(self, parent_frame):
        """Tworzy etykiety ze statystykami."""
        stats = {
            "Czas gry:": self.launcher.format_play_time(
                self.game_data.get("play_time", 0)
            ),
            ...
            "Data dodania:": (
                datetime.datetime.fromtimestamp(
                    self.game_data.get("date_added", 0)
                ).strftime("%Y-%m-%d")
                if self.game_data.get("date_added")
                else "Brak"
            ),
            "Ostatnio grane:": (
                datetime.datetime.fromtimestamp(
                    self.game_data.get("last_played", 0)
                ).strftime("%Y-%m-%d %H:%M")
                if self.game_data.get("last_played")
                else "Nigdy"
            ),
        ...
        for label, value in stats.items():
            ttk.Label(parent_frame, text=label, font=("Segoe UI", 9, "bold")).grid(
    ...
    def fetch_online_description_stub(self):
        ...
        messagebox.showinfo(
            "Wkrótce",
            "Funkcja pobierania opisu online zostanie dodana w przyszłości.",
            parent=self,
        )
    ...
    def create_action_buttons(self, parent_frame):
        ...
        launch_cmd = lambda p=default_profile: self.launcher.launch_game(
            self.game_name, profile=p
        )
        self.launch_button = ttk.Button(
            launch_btn_frame,
            text=launch_btn_text,
            style="Green.TButton",
            command=launch_cmd,
        )
        ...
        if len(profiles) > 1:
            ...
            profile_menu_btn = ttk.Menubutton(
                launch_btn_frame, text="▼", width=2, style="Toolbutton"
            )
            ...
            for profile in profiles:
                ...
                cmd = lambda p=profile: self.launcher.launch_game(
                    self.game_name, profile=p
                )
        ...
        mods_btn = ttk.Button(
            parent_frame, text="Mody", command=self.show_mods_for_game
        )
    ...
    def show_mods_for_game(self):
        ...
        self.launcher.root.after(
            100,
            lambda: self.launcher.extended_mod_manager.select_game_in_manager(
                self.game_name
            ),
        )
    ...
    def add_screenshots(self):
        ...
        screenshot_dir = os.path.join(
            IMAGES_FOLDER, "screenshots", re.sub(r'[\\/*?:"<>|]', "_", self.game_name)
        )
        ...
        for f_path in files:
            try:
                ...
                if not os.path.exists(destination_path):
                    ...
                    logging.warning(
                        f"Screenshot '{filename}' już istnieje w folderze docelowym. Pomijanie."
                    )
            except Exception as e:
        ...
        if added_paths:
            ...
            messagebox.showinfo(
                "Sukces", f"Dodano {len(added_paths)} screenshotów.", parent=self
            )
    ...
    def load_screenshots(self):
        ...
        normalized_manual_paths = {
            os.path.normcase(os.path.abspath(p)) for p in manual_paths_raw if p
        }
        normalized_auto_paths = {
            os.path.normcase(os.path.abspath(p)) for p in auto_paths_raw if p
        }
        ...
        for p in manual_paths_raw:
            if p:
                path_map[os.path.normcase(os.path.abspath(p))] = p
        for p in auto_paths_raw:
            norm_p = os.path.normcase(os.path.abspath(p)) if p else None
            if (
                norm_p and norm_p not in path_map
            ):  # Dodaj tylko jeśli nie ma już w mapie (z listy ręcznej)
                path_map[norm_p] = p

        # Ostateczna lista oryginalnych, unikalnych ścieżek do wyświetlenia, posortowana
        all_paths_to_display = sorted(
            [path_map[norm_p] for norm_p in all_normalized_paths if norm_p in path_map],
            key=str.lower,
        )

        if not all_paths_to_display:  # Użyj nowej listy
            ttk.Label(self.screenshot_inner_frame, text="(Brak screenshotów)").pack(
                pady=20
            )
            return
        ...
        try:
            ...
            if parent_width <= 1:
                parent_width = self.winfo_width() - 60
        except tk.TclError:
            parent_width = 600
        ...
        for i, img_path in enumerate(all_paths_to_display):  # Użyj nowej listy
            ...
            if not os.path.exists(img_path):
                logging.warning(
                    f"Plik screenshota nie istnieje: {img_path}. Pomijanie."
                )
            ...
            try:
                ...
                thumb_label.bind(
                    "<Button-1>", lambda e, p=img_path: self.view_screenshot(p)
                )

                tooltip_text = ""
                if is_manual and is_auto:
                    tooltip_text = (
                        "Screenshot dodany ręcznie (również znaleziony automatycznie)"
                    )
                elif is_manual:
                ...
                thumb_label.bind(
                    "<Button-3>",
                    lambda e, p=img_path, man=is_manual, auto=is_auto: self.screenshot_context_menu(
                        e, p, is_manual=man, is_auto=auto
                    ),
                )
                ...
                logging.error(
                    f"Błąd ładowania miniaturki screenshota '{img_path}': {e}"
                )
        ...
        self.screenshot_canvas.configure(
            scrollregion=self.screenshot_canvas.bbox("all")
        )
    ...
    def _remove_stale_screenshot_path(self, stale_path, was_manual, was_auto):
        ...
        logging.warning(
            f"Usuwanie nieistniejącej ścieżki screenshota z konfiguracji: {stale_path}"
        )
        path_removed = False
        if was_manual:
    ...
    def screenshot_context_menu(
        self, event, image_path, is_manual, is_auto
    ):  # Dodano is_auto
        """Wyświetla menu kontekstowe dla miniaturki screenshota."""
        menu = tk.Menu(self, tearoff=0, background="#2e2e2e", foreground="white")
        menu.add_command(
            label="Otwórz", command=lambda: self.view_screenshot(image_path)
        )
        menu.add_separator()
        # Dodaj opcję usunięcia z listy ręcznej, jeśli jest na niej
        if is_manual:
            menu.add_command(
                label="Usuń z listy ręcznej",
                command=lambda p=image_path: self.delete_screenshot(
                    p, delete_manual=True
                ),
            )
        # Dodaj opcję usunięcia z listy automatycznej, jeśli jest na niej
        if is_auto:
            menu.add_command(
                label="Usuń z listy automatycznej",
                command=lambda p=image_path: self.delete_screenshot(
                    p, delete_auto=True
                ),
            )
    ...
    def delete_screenshot(
        self, image_path_to_delete, delete_manual=False, delete_auto=False
    ):
        """Usuwa screenshot z odpowiednich list w konfiguracji."""
        if not delete_manual and not delete_auto:
            logging.warning(
                "Wywołano delete_screenshot bez określenia, z której listy usunąć."
            )
            return

        confirm_msg = (
            "Czy na pewno chcesz usunąć ten screenshot z wybranej listy(list)?"
        )
        ...
        if messagebox.askyesno("Potwierdź Usunięcie", confirm_msg, parent=self):
            ...
            try:
                ...
                if delete_manual:
                    ...
                    if image_path_to_delete in manual_screenshots:
                        ...
                        logging.info(
                            f"Usunięto screenshot z listy ręcznej: {image_path_to_delete}"
                        )
                        path_removed = True
                    else:
                        logging.warning(
                            f"Screenshot {image_path_to_delete} nie znaleziony na liście ręcznej."
                        )
                ...
                if delete_auto:
                    ...
                    if image_path_to_delete in auto_screenshots:
                        ...
                        logging.info(
                            f"Usunięto screenshot z listy automatycznej: {image_path_to_delete}"
                        )
                        path_removed = True
                    else:
                        logging.warning(
                            f"Screenshot {image_path_to_delete} nie znaleziony na liście automatycznej."
                        )
                ...
                if path_removed:
                    ...
                    messagebox.showinfo(
                        "Informacja",
                        "Screenshot nie został znaleziony na wybranej liście.",
                        parent=self,
                    )

            except Exception as e:
                logging.error(
                    f"Błąd podczas usuwania screenshota '{image_path_to_delete}': {e}"
                )
                messagebox.showerror(
                    "Błąd",
                    f"Nie udało się usunąć screenshota z listy:\n{e}",
                    parent=self,
                )
        else:
            ...
            messagebox.showinfo(
                "Informacja",
                "Tego screenshota nie można usunąć, ponieważ został znaleziony automatycznie lub już go usunięto z listy ręcznej.",
                parent=self,
            )
            logging.warning(
                f"Próba usunięcia screenshota, którego nie ma na liście ręcznej lub jest tylko auto: {image_path_to_delete}"
            )
    ...
    def _on_screenshot_mousewheel(self, event):
        ...
        try:
            ...
            KeyError,
            tk.TclError,
        ):  # Obsługa błędów, gdy kursor jest poza oknem lub nad menu
            return
    ...
    def _load_checklist(self):
        ...
        if (
            not hasattr(self, "checklist_tree")
            or not self.checklist_tree.winfo_exists()
        ):
            return
        ...
        self.checklist_tree.tag_configure(
            "done", foreground="gray", font=("Segoe UI", 9, "overstrike")
        )  # Przekreślone dla ukończonych
        self.checklist_tree.tag_configure(
            "pending", foreground="white", font=("Segoe UI", 9)
        )  # Normalne dla oczekujących

        checklist_data = self.game_data.get("checklist", [])
        logging.debug(
            f"Ładowanie checklisty dla {self.game_name}: {len(checklist_data)} zadań."
        )
        ...
        for idx, item_data in enumerate(checklist_data):
            ...
            self.checklist_tree.insert(
                "", "end", iid=iid, values=(status_char, task_text), tags=(tag,)
            )
    ...
    def _toggle_task_status(self, item_iid):
        ...
        try:
            ...
            if 0 <= index < len(checklist):
                ...
                tag = "done" if is_done else "pending"
                self.checklist_tree.item(
                    item_iid,
                    values=(status_char, checklist[index].get("task")),
                    tags=(tag,),
                )
                logging.info(
                    f"Zmieniono status zadania {index} na {is_done} dla gry {self.game_name}"
                )
            else:
                logging.warning(
                    f"Próba przełączenia statusu dla nieprawidłowego indeksu {index} w checkliście."
                )
        except (ValueError, IndexError) as e:
            logging.error(
                f"Błąd podczas przełączania statusu zadania (iid={item_iid}): {e}"
            )

    def _add_task(self):
        """Dodaje nowe zadanie do checklisty."""
        new_task_desc = simpledialog.askstring(
            "Nowe Zadanie", "Wpisz opis nowego zadania:", parent=self
        )
        if new_task_desc and new_task_desc.strip():
            ...
            logging.info(
                f"Dodano nowe zadanie '{new_task_desc}' dla gry {self.game_name}"
            )
    ...
    def _edit_task(self):
        ...
        if not selection:
            messagebox.showwarning(
                "Brak zaznaczenia",
                "Zaznacz zadanie, które chcesz edytować.",
                parent=self,
            )
            return
        ...
        try:
            ...
            if 0 <= index < len(checklist):
                ...
                if new_desc and new_desc.strip() and new_desc.strip() != current_desc:
                    ...
                    self._load_checklist()  # Odśwież całą listę
                    logging.info(
                        f"Zmieniono opis zadania {index} na '{new_desc.strip()}' dla gry {self.game_name}"
                    )
            else:
                logging.warning(
                    f"Próba edycji nieprawidłowego indeksu {index} w checkliście."
                )
        except (ValueError, IndexError) as e:
    ...
    def _delete_task(self):
        ...
        if not selection:
            messagebox.showwarning(
                "Brak zaznaczenia", "Zaznacz zadanie, które chcesz usunąć.", parent=self
            )
            return
        selected_iid = selection[0]
        ...
        try:
            ...
            if 0 <= index < len(checklist):
                ...
                if messagebox.askyesno(
                    ...
                    self._load_checklist()  # Odśwież całą listę
                    logging.info(
                        f"Usunięto zadanie {index} ('{task_desc}') dla gry {self.game_name}"
                    )
            else:
                logging.warning(
                    f"Próba usunięcia nieprawidłowego indeksu {index} w checkliście."
                )
        except (ValueError, IndexError) as e:
    ...
    def update_basic_info(self):
        ...
        self.developer_label.config(
            text=f"Deweloper: {', '.join(devs) if devs else '-'}"
        )
    ...
    def open_website(self):
        ...
        if url:
            try:
                ...
                messagebox.showerror(
                    "Błąd",
                    f"Nie można otworzyć adresu URL:\n{url}\n\nBłąd: {e}",
                    parent=self,
                )
    ...
    def create_launcher_stats_labels(self, parent_frame):
        ...
        for i, (label, value) in enumerate(stats.items()):
            ttk.Label(parent_frame, text=label, font=("Segoe UI", 8, "bold")).grid(
                row=row, column=col * 2, sticky="w", padx=2, pady=1
            )
            ttk.Label(parent_frame, text=value, font=("Segoe UI", 8)).grid(
                row=row, column=col * 2 + 1, sticky="w", padx=2, pady=1
            )
            row += 1
    ...
    def update_api_info(self):
        ...
        self.platforms_label.config(
            text=f"Platformy: {', '.join(platforms) if platforms else '-'}"
        )
        self.genres_api_label.config(
            text=f"Gatunki (API): {', '.join(genres_api) if genres_api else '-'}"
        )
        self.tags_api_label.config(
            text=f"Tagi (API): {', '.join(tags_api) if tags_api else '-'}"
        )
    ...
    def update_requirements(self):
        ...
        self.req_text.insert(
            "1.0", req_str if req_str else "(Brak danych o wymaganiach)"
        )
    ...
    def create_launch_button_with_profiles(
        self, parent_frame, start_column=1
    ):  # Dodano start_column
        ...
        launch_btn_frame.grid(
            row=0, column=start_column, padx=2, pady=2, sticky="nsew"
        )  # Użyj start_column
        ...
        if len(profiles) == 1 and default_profile.get("name", "").lower() == "default":
            ...
            launch_btn_text = (
                f"Uruchom ({default_profile.get('name', '?')})"  # Krótszy format
            )
    ...
    def _on_details_fetched(self, result):
        logging.debug(
            f"Otrzymano wynik pobierania RAWG dla {self.game_name}: Success={result['success']}"
        )
        if not self.winfo_exists():
            logging.warning(
                "Okno szczegółów zamknięte przed zakończeniem pobierania RAWG."
            )
            return
        ...
        if result["success"] and result["data"]:
            ...
            downloaded_cover_path = fetched_data.get(
                "downloaded_cover_path"
            )  # Będzie None, jeśli pobieranie zawiodło

            if downloaded_cover_path and os.path.exists(
                downloaded_cover_path
            ):  # Sprawdź, czy mamy poprawną ścieżkę
                ...
                if force_cover_update:  # Wymuszone przez użytkownika
                    ...
                    logging.info(
                        f"Wymuszone ustawienie okładki RAWG dla '{self.game_name}'."
                    )
                elif self.game_data.get("_auto_cover"):  # Obecna to placeholder
                    should_set_cover = True
                    logging.info(
                        f"Ustawianie okładki RAWG dla '{self.game_name}' (zastępuje placeholder)."
                    )
                elif not current_cover or not os.path.exists(
                    current_cover
                ):  # Brak okładki lub stara nie istnieje
                    should_set_cover = True
                    logging.info(
                        f"Ustawianie pobranej okładki RAWG dla '{self.game_name}', ponieważ brak istniejącej."
                    )
                ...
                if should_set_cover:
                    ...
                    logging.info(
                        f"Gra '{self.game_name}' ma już własną okładkę. Pobrano RAWG, ale nie ustawiono jako aktywnej."
                    )

            elif (
                downloaded_cover_path
            ):  # Ścieżka była zwrócona, ale plik nie istnieje (błąd zapisu?)
                logging.error(
                    f"Pobrana ścieżka okładki RAWG '{downloaded_cover_path}' nie wskazuje na istniejący plik."
                )
            ...
            if data_changed:
                logging.info(
                    f"Dane gry '{self.game_name}' zostały zaktualizowane. Zapisywanie konfiguracji."
                )
                save_config(self.launcher.config)
            else:
                logging.info(
                    f"Dane z RAWG dla '{self.game_name}' nie różniły się od istniejących."
                )
            ...
            if cover_set_from_rawg:
                try:
                    load_photoimage_from_path.cache_clear()
                    logging.info(
                        "Wyczyszczono cache load_photoimage_from_path z powodu ustawienia okładki z RAWG."
                    )
                    self.launcher.root.after(
                        150,
                        lambda gn=self.game_name: self.launcher._force_refresh_tile(gn),
                    ...
                    logging.error(
                        f"Nie udało się wyczyścić cache PhotoImage po pobraniu okładki RAWG: {e}"
                    )

            if (
                data_changed or cover_set_from_rawg
            ):  # Pokaż sukces jeśli cokolwiek się zmieniło lub ustawiono okładkę
                messagebox.showinfo(
                    "Sukces", "Pobrano i zaktualizowano dane gry.", parent=self
                )
            else:
                messagebox.showinfo(
                    "Informacja",
                    "Pobrano dane gry, ale nie wymagały one aktualizacji.",
                    parent=self,
                )
            ...
            messagebox.showerror(
                "Błąd Pobierania",
                f"Nie udało się pobrać danych z RAWG:\n{result['error']}",
                parent=self,
            )
        else:
            messagebox.showerror(
                "Błąd", "Wystąpił nieznany błąd podczas pobierania danych.", parent=self
            )
    ...
    def remove_cover(self):
        ...
        if original_cover_path:
            ...
            if os.path.exists(original_cover_path) and os.path.dirname(
                os.path.abspath(original_cover_path)
                ...
                try:
                    ...
                    logging.error(
                        f"Nie udało się usunąć pliku okładki {original_cover_path}: {e}"
                    )
            ...
            self.refresh_details_data()  # Odśwież widok szczegółów
            self.launcher.root.after(
                150, lambda gn=self.game_name: self.launcher._force_refresh_tile(gn)
            )
        else:
    ...
    def fetch_cover_from_rawg(self, force=False):
        ...
        if rawg_cover_path and os.path.exists(rawg_cover_path) and not force:
            ...
            if self.game_data.get("cover_image") != rawg_cover_path:
                ...
                messagebox.showinfo(
                    "Informacja", "Okładka z RAWG jest już ustawiona.", parent=self
                )
        else:
            # Nie mamy zapisanej lub chcemy wymusić pobranie nowej
            logging.info(
                f"Rozpoczynam pobieranie danych RAWG (force_cover={force}) dla {self.game_name}, aby uzyskać okładkę."
            )
    ...
    def show_cover_context_menu(self, event):
        ...
        menu = tk.Menu(self, tearoff=0, background="#2e2e2e", foreground="white")
        menu.add_command(
            label="Ustaw Własną Okładkę...", command=self.select_custom_cover
        )
        # Opcja użycia okładki RAWG tylko jeśli jest zapisana ścieżka
        if self.game_data.get("cover_rawg"):
            menu.add_command(
                label="Użyj Okładki z RAWG",
                command=lambda: self.fetch_cover_from_rawg(force=False),
            )  # force=False tylko użyje zapisanej
        menu.add_command(
            label="Pobierz Ponownie z RAWG",
            command=lambda: self.fetch_cover_from_rawg(force=True),
        )  # force=True wymusi pobranie
