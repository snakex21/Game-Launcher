...
class ExtendedModManager:
    def __init__(self, parent_launcher, root):
        ...
        ttk.Label(
            ...
            row=0, column=0
        )  # Wyśrodkowany dzięki weight=1 w header_frame
        ...
        self.profile_menu = ttk.OptionMenu(
            left_panel, self.profile_var, self.profile_var.get(), "default"
        )
        self.profile_menu.pack(pady=5, fill="x")

        ttk.Button(left_panel, text="Utwórz Profil", command=self.create_profile).pack(
            pady=5, fill="x"
        )
        ttk.Button(left_panel, text="Usuń Profil", command=self.delete_profile).pack(
            pady=5, fill="x"
        )
        ...
        scrollbar = ttk.Scrollbar(
            right_panel, orient="vertical", command=self.mods_tree.yview
        )
        ...
        for idx, (text, command) in enumerate(buttons):
            ttk.Button(buttons_frame, text=text, command=command).grid(
                row=0, column=idx, sticky="ew", padx=2
            )

        # Przycisk Odśwież na dole osobno
        refresh_btn = ttk.Button(
            content_frame, text="Odśwież", command=self.load_current_game_profile
        )
        refresh_btn.grid(row=3, column=0, columnspan=2, pady=5, sticky="e")

        # Inicjalizacja
        self.last_mod_locations = self.launcher.config.setdefault(
            "last_mod_locations", {}
        )
    ...
    def select_game_in_manager(self, game_name_to_select):
        ...
        try:
            ...
            logging.error(
                "Błąd TclError podczas pobierania opcji menu w select_game_in_manager."
            )
        ...
        if game_name_to_select in options:
            try:
                ...
                game_found = True
                logging.info(
                    f"Programowo wybrano grę '{game_name_to_select}' w menedżerze modów."
                )
            except Exception as e:
                logging.error(
                    f"Błąd podczas ustawiania game_var lub wywoływania on_game_change dla '{game_name_to_select}': {e}"
                )
        else:
            logging.warning(
                f"Nie można znaleźć gry '{game_name_to_select}' wśród opcji dropdown menedżera modów: {options}"
            )
    ...
    def deactivate_mod(self):
        ...
        if not mod_info.get("active", False):
            messagebox.showinfo(
                "Informacja", f"Mod '{mod_name}' jest już dezaktywowany."
            )
            return
        ...
        if not backup_folder or not os.path.isdir(backup_folder):
            messagebox.showerror(
                "Błąd", f"Nie znaleziono backupu dla moda '{mod_name}'."
            )
            return
            ...
            for rel_path in mod_info.get("files_changed", []):
                ...
                if os.path.exists(backed_file):
                    try:
                        ...
                        logging.error(
                            f"Nie udało się przywrócić {original_in_game}: {e}"
                        )
    ...
    def on_game_change(self, selected_game):
        ...
        if "default" not in profiles:
            ...
            game_data["profiles"]["default"] = {"mods": {}, "load_order": []}
            self.save_mods_data()
    ...
    def load_current_game_profile(self):
        ...
        profile_data = game_data["profiles"].get(
            profile, {"mods": {}, "load_order": []}
        )
    ...
    def create_profile(self):
        ...
        profile_name = tk.simpledialog.askstring(
            "Nazwa profilu", "Podaj nazwę nowego profilu:"
        )
        if not profile_name:
            return
        ...
        game_data["profiles"][profile_name] = {"mods": {}, "load_order": []}
        self.save_mods_data()
    ...
    def delete_profile(self):
        ...
        confirm = messagebox.askyesno(
            "Potwierdzenie", f"Czy na pewno chcesz usunąć profil '{profile}'?"
        )
        if confirm:
    ...
    def install_mod_zip_flow(self):
        ...
        zip_file = filedialog.askopenfilename(
            title="Wybierz plik ZIP z modem", filetypes=[("Archiwa ZIP", "*.zip")]
        )
        if not zip_file:
            return  # Anulowano
        ...
        for location in last_locations:
            btn = ttk.Button(
                last_locations_frame,
                text=location,
                command=lambda loc=location: self.select_location(
                    location_window, loc, zip_file, mod_name
                ),
            )
        ...
        new_location_btn = ttk.Button(
            location_window,
            text="Wybierz inną lokalizację...",
            command=lambda: self.select_new_location(
                location_window, zip_file, game, mod_name
            ),
        )
    ...
    def install_mod_from_zip(self, zip_path, mods_folder, mod_name):
        ...
        game_data = self.mods_data.setdefault(game, {"profiles": {}})
        profile_data = game_data["profiles"].setdefault(
            profile, {"mods": {}, "load_order": []}
        )

        # Zapytaj czy mod ma być od razu aktywny
        make_active = messagebox.askyesno(
            "Aktywować od razu?", "Czy chcesz od razu aktywować ten mod?"
        )
        ...
        messagebox.showinfo(
            "Sukces", f"Mod '{mod_name}' został zainstalowany (aktywny: {make_active})."
        )
    ...
    def install_mod(self):
        ...
        for location in last_locations:
            btn = ttk.Button(
                last_locations_frame,
                text=location,
                command=lambda loc=location: self.select_mod_location(
                    location_window, loc
                ),
            )
            btn.pack(side="top", fill="x", padx=5, pady=2)
    ...
    def process_mod_installation(self, mod_dir):
        ...
        if not os.path.isdir(game_folder):
            messagebox.showerror(
                "Błąd", "Folder gry nie istnieje - sprawdź ścieżkę exe_path."
            )
            return
        ...
        if mod_name in profile_data["mods"]:
            messagebox.showwarning(
                "Uwaga", f"Mod '{mod_name}' jest już zainstalowany w tym profilu."
            )
            return
        ...
        backup_folder = os.path.join(
            game_folder, f"_backup_{mod_name}_{int(time.time())}"
        )
    ...
    def uninstall_mod(self):
        ...
        if not selection:
            messagebox.showwarning(
                "Błąd", "Nie wybrano żadnego moda do odinstalowania."
            )
            return
        ...
        if backup_folder and os.path.isdir(backup_folder):
            try:
                ...
                messagebox.showerror(
                    "Błąd", f"Nie udało się przywrócić oryginalnych plików: {e}"
                )
                return
    ...
    def reorder_load_list(self, game, profile):
        ...
        sorted_mods = sorted(
            profile_data["mods"].items(), key=lambda kv: kv[1]["priority"], reverse=True
        )
