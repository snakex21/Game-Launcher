...
class SaveManager:
    def __init__(self, parent, game_name, game_data, launcher_instance):
        ...
        ttk.Label(
            self.top, text=f"Zarządzanie zapisami - {game_name}", font=("Helvetica", 14)
        ).pack(pady=10)
        ...
        list_frame.pack(
            pady=5, fill=tk.BOTH, expand=True, padx=10
        )  # Dodano expand=True, fill=tk.BOTH
        ...
        self.saves_tree.column(
            "Nazwa Folderu", width=200, anchor="w"
        )  # Pozostała szerokość

        # Scrollbar (bez zmian, ale dla Treeview)
        scrollbar = ttk.Scrollbar(
            list_frame, orient="vertical", command=self.saves_tree.yview
        )
        ...
        ttk.Button(
            ...
            state=tk.NORMAL if self.is_save_path_valid else tk.DISABLED,
        ...
        ttk.Button(button_frame, text="Edytuj Zapis", command=self.edit_save).grid(
            row=1, column=0, padx=5, pady=3, sticky="ew"
        )  # Edycja nazwy backupu zawsze możliwa
        ttk.Button(button_frame, text="Usuń Zapis", command=self.delete_save).grid(
            row=1, column=1, padx=5, pady=3, sticky="ew"
        )  # Usuwanie backupu zawsze możliwe

        open_save_folder_btn = ttk.Button(
            button_frame,
            text="Otwórz Folder Zapisów Gry",
            command=lambda: self.launcher._open_folder(
                self.save_path
            ),  # Użyj metody z launchera
            state=(
                tk.NORMAL if self.is_save_path_valid else tk.DISABLED
            ),  # Stan zależny od poprawności ścieżki
        )
        open_save_folder_btn.grid(
            row=2, column=0, columnspan=2, padx=5, pady=8, sticky="ew"
        )  # Rozciągnij na dwie kolumny

        ttk.Button(button_frame, text="Zamknij", command=self.top.destroy).grid(
            row=3, column=0, columnspan=2, padx=5, pady=(15, 5), sticky="ew"
        )

    def update_saves_list(self):
        """Wczytuje zapisy ręczne i automatyczne do Treeview, sortując je."""
        if not hasattr(self, "saves_tree") or not self.saves_tree.winfo_exists():
            return
        ...
        if os.path.isdir(autosave_path):
            try:
                ...
                self.saves_tree.tag_configure(
                    "autosave", foreground="lightblue", font=("Segoe UI", 9, "italic")
                )  # Dodano font dla lepszej czytelności
            ...
            for save_name_folder in os.listdir(saves_folder):
                ...
                if os.path.isdir(save_full_path):
                    try:
                        ...
                        percent = 0
                        match = re.search(
                            r"Save_(\d+)%", save_name_folder
                        )  # Szukaj procentu w nazwie folderu
                        if match:
                            percent = int(match.group(1))
                        manual_saves_data.append(
                            {
                                "name": save_name_folder,
                                "mtime": mtime,
                                "percent": percent,
                            }
                        )
                        ...
                        manual_saves_data.append(
                            {"name": save_name_folder, "mtime": 0, "percent": 0}
                        )  # Fallback

            manual_saves_data.sort(
                key=lambda item: (item["mtime"], item["percent"]), reverse=True
            )

            for save_data_item in manual_saves_data:
                save_date_item = datetime.datetime.fromtimestamp(
                    save_data_item["mtime"]
                )
                self.saves_tree.insert(
                    "",
                    "end",
                    iid=save_data_item["name"],
                    values=(
    ...
    def create_save(self):
        ...
        if not self.is_save_path_valid:  # Użyj flagi sprawdzonej w __init__
            messagebox.showwarning(
                "Błąd",
                "Ścieżka do zapisów gry jest nieprawidłowa lub nie istnieje.",
                parent=self.top,
            )
            return
        ...
        percent = simpledialog.askinteger(
            "Procent ukończenia",
            "Podaj procent ukończenia gry dla tego zapisu:",
            parent=self.top,
        )
        if percent is not None:
            ...
            if os.path.exists(destination):
                messagebox.showerror(
                    "Błąd",
                    f"Zapis o nazwie '{save_name}' już istnieje.",
                    parent=self.top,
                )
                return
            ...
            if success:
                ...
                messagebox.showinfo(
                    "Rozpoczęto",
                    f"Rozpoczęto tworzenie zapisu '{save_name}'.",
                    parent=self.top,
                )
    ...
    def _get_selected_save_info(self):
        ...
        if not selection:
            return None, None, False  # Zwraca (ścieżka, nazwa dla logu, czy_autosave)
    ...
    def load_save(self):
        ...
        source_path, save_name_for_log, _ = (
            self._get_selected_save_info()
        )  # Nie potrzebujemy tu is_autosave osobno

        if not source_path:  # Jeśli nic nie zaznaczono
            messagebox.showwarning(
                "Błąd", "Nie wybrano żadnego zapisu.", parent=self.top
            )
            return
        ...
        if not os.path.isdir(source_path):
            messagebox.showerror(
                "Błąd",
                f"Ścieżka źródłowa zapisu '{source_path}' nie istnieje lub nie jest folderem.",
                parent=self.top,
            )
            self.update_saves_list()
            return
        ...
        if not self.is_save_path_valid:
            messagebox.showerror(
                "Błąd",
                "Ścieżka do zapisów gry jest nieprawidłowa.\nUstaw ją w edycji gry.",
                parent=self.top,
            )
            return
        ...
        if messagebox.askyesno(
            "Potwierdź Wczytanie",
            f"Czy na pewno chcesz wczytać zapis '{save_name_for_log}'?\nSpowoduje to nadpisanie aktualnych zapisów gry!",
            parent=self.top,
        ):
            success = self.launcher._copy_or_delete_with_progress(
                operation_type="copy",
            ...
            if success:
                messagebox.showinfo(
                    "Rozpoczęto",
                    f"Rozpoczęto wczytywanie zapisu '{save_name_for_log}'.",
                    parent=self.top,
                )
    ...
    def edit_save(self):
        ...
        selected_save_path, old_save_name_folder, is_autosave = (
            self._get_selected_save_info()
        )

        if not selected_save_path:  # Nic nie zaznaczono
            messagebox.showwarning(
                "Błąd", "Nie wybrano zapisu do edycji.", parent=self.top
            )
            return
        ...
        if is_autosave:
            messagebox.showinfo(
                "Informacja",
                "Nie można zmienić nazwy automatycznego zapisu.",
                parent=self.top,
            )
            return
        ...
        percent = simpledialog.askinteger(
            "Edytuj Procent Ukończenia",
            "Podaj nowy procent ukończenia dla tego zapisu (tylko dla nazwy):",
            parent=self.top,
        )
        if percent is not None:
            try:
                ...
                if new_save_folder_name == old_save_name_folder:
                    return

                saves_base_folder = os.path.join(self.backup_path, "saves")
                new_full_path_check = os.path.join(
                    saves_base_folder, new_save_folder_name
                )

                if os.path.exists(new_full_path_check):
                    messagebox.showerror(
                        "Błąd",
                        f"Zapis o nazwie '{new_save_folder_name}' już istnieje.",
                        parent=self.top,
                    )
                    return

                old_full_path = os.path.join(
                    saves_base_folder, old_save_name_folder
                )  # To samo co selected_save_path

                os.rename(old_full_path, new_full_path_check)
                messagebox.showinfo(
                    "Sukces", "Nazwa zapisu została zaktualizowana.", parent=self.top
                )
                self.update_saves_list()
            except IndexError:
                messagebox.showerror(
                    "Błąd Nazwy",
                    "Nie można przetworzyć starej nazwy zapisu do zmiany procentu.",
                    parent=self.top,
                )
            except Exception as e:
                logging.exception(
                    f"Błąd podczas zmiany nazwy zapisu '{old_save_name_folder}': {e}"
                )
                messagebox.showerror(
                    "Błąd",
                    f"Nie udało się zaktualizować nazwy zapisu:\n{e}",
                    parent=self.top,
                )
    ...
    def delete_save(self):
        ...
        if not path_to_delete:
            messagebox.showwarning(
                "Błąd", "Nie wybrano zapisu do usunięcia.", parent=self.top
            )
            return
        ...
        if not os.path.isdir(path_to_delete):
            messagebox.showerror(
                "Błąd",
                f"Folder zapisu '{save_name_for_log}' nie istnieje:\n{path_to_delete}",
                parent=self.top,
            )
            self.update_saves_list()
            return
        ...
        if messagebox.askyesno(
            "Potwierdź Usunięcie",
            f"Czy na pewno chcesz trwale usunąć zapis '{save_name_for_log}'?",
            parent=self.top,
        ):
            success = self.launcher._copy_or_delete_with_progress(
            ...
            if (
                success
            ):  # Jeśli operacja została poprawnie zainicjowana (nie oznacza to jeszcze zakończenia wątku)
                messagebox.showinfo(
                    "Rozpoczęto",
                    f"Rozpoczęto usuwanie zapisu '{save_name_for_log}'.\nLista zostanie odświeżona po zakończeniu.",
                    parent=self.top,
                )
